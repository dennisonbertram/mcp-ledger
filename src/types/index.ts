...
export const GetSolanaAddressSchema = z.object({
  derivationPath: z.string().optional().describe('BIP32 derivation path for Solana (default: "44\'/501\'/0\'/0\'")'),
  display: z.boolean().optional().default(false).describe('Display address on Ledger device for confirmation'),
  network: z.enum(['solana-mainnet', 'solana-devnet', 'solana-testnet']).optional().default('solana-mainnet').describe('Solana network'),
});

export const GetSolanaBalanceSchema = z.object({
  address: z.string().describe('Solana address to check balance for (Base58 format)'),
  network: z.enum(['solana-mainnet', 'solana-devnet', 'solana-testnet']).optional().default('solana-mainnet').describe('Solana network to check balance on'),
});

export const GetSplTokenBalancesSchema = z.object({
  address: z.string().describe('Solana address to check SPL token balances for'),
  mintAddresses: z.array(z.string()).optional().describe('Specific SPL token mint addresses to check (optional)'),
  network: z.enum(['solana-mainnet', 'solana-devnet', 'solana-testnet']).optional().default('solana-mainnet').describe('Solana network to check balances on'),
});

export const CraftSolanaTransactionSchema = z.object({
  type: z.enum(['sol_transfer', 'spl_token_transfer', 'token_approval', 'custom']).describe('Type of Solana transaction to craft'),
  network: z.enum(['solana-mainnet', 'solana-devnet', 'solana-testnet']).optional().default('solana-mainnet').describe('Solana network for the transaction'),
  // SOL transfer params
  fromAddress: z.string().optional().describe('Sender address (for SOL transfers)'),
  toAddress: z.string().optional().describe('Recipient address'),
  amount: z.string().optional().describe('Amount to transfer (in SOL or token units)'),
  // SPL token params
  mintAddress: z.string().optional().describe('SPL token mint address (for token transfers)'),
  decimals: z.number().optional().describe('Token decimals (required for token transfers)'),
  createRecipientAccount: z.boolean().optional().default(false).describe('Create recipient token account if it doesn\'t exist'),
  // Token approval params
  spenderAddress: z.string().optional().describe('Spender address (for token approvals)'),
  // Custom transaction params
  instructions: z.array(z.object({
    programId: z.string().describe('Program ID to invoke'),
    accounts: z.array(z.object({
      pubkey: z.string().describe('Account public key'),
      isSigner: z.boolean().describe('Whether account is a signer'),
      isWritable: z.boolean().describe('Whether account is writable'),
    })).describe('Accounts required by the instruction'),
    data: z.string().describe('Instruction data (hex encoded)'),
  })).optional().describe('Custom instructions (for custom transactions)'),
  // Common params
  priorityFee: z.number().optional().describe('Priority fee in microlamports'),
  computeUnits: z.number().optional().describe('Compute units limit'),
  memo: z.string().optional().describe('Optional memo to include'),
});

export const SignSolanaTransactionSchema = z.object({
  transactionData: z.string().describe('Base64 encoded Solana transaction data'),
  derivationPath: z.string().optional().default("44'/501'/0'/0'").describe('BIP32 derivation path for signing'),
  network: z.enum(['solana-mainnet', 'solana-devnet', 'solana-testnet']).optional().default('solana-mainnet').describe('Solana network for the transaction'),
});

export const SendSolSchema = z.object({
  toAddress: z.string().describe('Recipient Solana address (Base58 format)'),
  amount: z.string().describe('Amount in SOL to send (e.g., "0.1")'),
  network: z.enum(['solana-mainnet', 'solana-devnet', 'solana-testnet']).optional().default('solana-mainnet').describe('Solana network to send on'),
  derivationPath: z.string().optional().default("44'/501'/0'/0'").describe('BIP32 derivation path for signing'),
  priorityFee: z.number().optional().describe('Priority fee in microlamports'),
  memo: z.string().optional().describe('Optional memo to include'),
});
...
export const SendSplTokenSchema = z.object({
  ...
  mintAddress: z.string().describe('SPL token mint address'),
  amount: z.string().describe('Amount in token units to send'),
  network: z.enum(['solana-mainnet', 'solana-devnet', 'solana-testnet']).optional().default('solana-mainnet').describe('Solana network to send on'),
  derivationPath: z.string().optional().default("44'/501'/0'/0'").describe('BIP32 derivation path for signing'),
  createRecipientAccount: z.boolean().optional().default(false).describe('Create recipient token account if needed'),
  memo: z.string().optional().describe('Optional memo to include'),
});

export const AnalyzeSolanaFeesSchema = z.object({
  network: z.enum(['solana-mainnet', 'solana-devnet', 'solana-testnet']).optional().default('solana-mainnet').describe('Solana network to analyze fees for'),
  transactionType: z.enum(['sol_transfer', 'spl_token_transfer', 'complex']).optional().describe('Type of transaction to estimate fees for'),
  priorityLevel: z.enum(['low', 'medium', 'high']).optional().default('medium').describe('Priority level preference'),
});
